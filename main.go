...
func (c *hetznerDNSProviderSolver) buildConfig(ch *v1alpha1.ChallengeRequest) (internal.Config, error) {
	...
	if err != nil {
		return internal.Config{}, fmt.Errorf(
			"unable to get key %q from secret `%s/%s`; %v",
			secretKey, secretName, ch.ResourceNamespace, err,
		)
	}
...
func callApi(url, method string, body io.Reader, config internal.Config) ([]byte, error) {
	...
	text := fmt.Sprintf(
		"Error calling API status: %s url: %s method: %s body: %s",
